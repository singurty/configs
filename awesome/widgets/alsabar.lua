...
local function factory(args)
	...
	local high_image = os.getenv("HOME") .. '/.config/awesome/icons/audio-volume-high-symbolic.svg'
	local medium_image = os.getenv("HOME") .. '/.config/awesome/icons/audio-volume-medium-symbolic.svg'
    ...
    function alsabar.update(callback)
        helpers.async(format_cmd, function(mixer)
            ...
            if vol ~= alsabar._current_level or playback ~= alsabar._playback then
                ...
                alsabar.bar:get_children_by_id('bar')[1]:set_value(alsabar._current_level)
                if alsabar._current_level == 0 or playback == "off" then
					...
					if alsabar._current_level < 30 then
						alsabar.bar:get_children_by_id('icon')[1].image = low_image
					elseif alsabar._current_level < 70 then
						alsabar.bar:get_children_by_id('icon')[1].image = medium_image
					else
						alsabar.bar:get_children_by_id('icon')[1].image = high_image
					end
                end
    ...
    alsabar.timer = helpers.newtimer(string.format("alsabar-%s-%s", alsabar.cmd, alsabar.channel), timeout, alsabar.update, false, true)
